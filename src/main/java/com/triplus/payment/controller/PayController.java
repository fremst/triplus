...
import com.triplus.payment.service.OrderService;
import com.triplus.payment.service.PayCancelService;
import com.triplus.payment.service.PaymentService;
...
public class PayController {
    ...
    @Autowired
    PayCancelService payCancelServie;

    @PostMapping("/pcpay")
    public String PayRequest(Model model, PcPayRequestDto pcPayRequestDto, OrderDto orderDto) {

        orderDto.setOid(pcPayRequestDto.getOid());
        orderService.insert(orderDto); // DB 대신 FE로 넘겨서 store에 저장하는 것 고려
        model.addAttribute("pay", pcPayRequestDto);
        return "payform";
    }
            ...
            if ("0000".equals(paramMap.get("resultCode"))) { // 인증이 성공인 경우
                ...
                try {
                    ...
                    if ("0000".equals(resultMap.get("resultCode")) && secureSignature.equals(resultMap.get("authSignature"))) {    //결제보안 강화 2016-05-18
                        ...
                        paymentService.insert(new PaymentDto("aa", "aa", "aa", "aa", "aa", "aa", "aa"));
    ...
    public void pcRefund(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String tid = "StdpayCARDINIpayTest20220811102608608514";  // 40byte 승인 TID 입력
            String msg = "거래취소요청";
